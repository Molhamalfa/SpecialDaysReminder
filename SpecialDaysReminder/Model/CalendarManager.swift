...
class CalendarManager: ObservableObject {
    ...
    func requestCalendarAuthorization(completion: @escaping (Bool, Error?) -> Void) {
        ...
        eventStore.requestFullAccessToEvents { [weak self] granted, error in
            // NOTE: The warning "Variable 'self' was written to, but never read" on this line is a common false positive.
            // 'self' is correctly used below within the DispatchQueue.main.async block to ensure the completion
            // handler is executed on the main thread, within the context of this instance.
